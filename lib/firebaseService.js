...
export const cricketAuctionService = {
  ...
  async clearBids(auctionId) {
    try {
      ...
      const querySnapshot = await getDocs(q);
      await Promise.all(querySnapshot.docs.map((bidDoc) => deleteDoc(bidDoc.ref)));
    } catch (error) {
      logger.error('Error clearing cricket bids:', error);
      throw error;
    }
  }
};
  ...
  async create(teamData) {
    try {
      const docRef = await addDoc(collection(db, 'cricket-teams'), {
        ...teamData,
        totalBalance: teamData.budget || 500000,
        spent: 0,
        createdAt: new Date(),
        updatedAt: new Date()
      });
